...
# Installer TermChat untuk Termux. Jalankan: bash install.sh
set -e
echo "==> Memasang dependensi..."
pkg install -y python
pip install --upgrade textual httpx
mkdir -p "$HOME/termchat"
cat > "$HOME/termchat/termchat.py" <<'PYEOF'
#!/usr/bin/env python3
...
exec python "$HOME/termchat/termchat.py" "$@"
EOF
chmod +x "$PREFIX/bin/termchat"
echo "==> Selesai. Jalankan dengan perintah: termchat"
